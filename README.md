...
The current runtime services are Technitium DNS Server, Caddy, Homepage, and OpenSpeedTest. Technitium provides DNS, Caddy is the local HTTP reverse proxy, Homepage is the dashboard, and OpenSpeedTest provides LAN speed testing.
...
`infrastructure/` contains platform services such as DNS and the Caddy reverse proxy. `applications/` contains user-facing app stacks. Homepage is the dashboard application, and OpenSpeedTest is the speed test application.
...
Expected result: `dns.home.arpa`, `homepage.home.arpa`, and `speedtest.home.arpa` resolve to `192.168.178.2`, Caddy answers on port `80`, and the Web UIs are reachable through Caddy.
