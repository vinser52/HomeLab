...
The initial `Network Gateway Overview` dashboard shows FritzBox target health, WAN link state, current WAN download and upload speed, WAN link capacity and utilization, total WAN traffic, WAN packet rate, router uptime, Wi-Fi channel, and Wi-Fi client metrics. fritz-exporter uses the FritzBox TR-064 API with credentials stored only in local `.env`.
